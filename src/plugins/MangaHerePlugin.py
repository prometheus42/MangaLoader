...
class MangaHerePlugin(PluginBase.PluginBase):
    ...
    def getImage(self, image):
        ...
        url = self.__domain + '/' + self.__getInternalName(manga.name) + '/c' + (
        '%03d' % chapter.chapterNo) + '/' + str(image.imageNo) + '.html'
        ...
        if parser.targetCount > 1:
            logger.warning(
                str(parser.targetCount) + ' images found in MangaHere site, maybe the chapter is not available.')
            return False
